...
void solve(){
    ...
    v64 b(m); forn(i,m) cin >> b[i];
    vvp64 dp(n + 1, vp64(m + 1));
    forn(i,n) forn(j,m){
        if(a[i] == b[j]){
            dp[i + 1][j + 1].fi = 1 + dp[i][j].fi;
            dp[i + 1][j + 1].se = 0;
        }else{
            if(dp[i + 1][j] > dp[i][j + 1]){
                // up > left
                dp[i + 1][j + 1].fi = dp[i + 1][j].fi;
                dp[i + 1][j + 1].se = 1;
            }else{
                dp[i + 1][j + 1].fi = dp[i][j + 1].fi;
                dp[i + 1][j + 1].se = 2;
            }
        }
    }
    ...
    for(ll i = sz(res) - 1; i >= 0; i-- ) cout << res[i] << " \n"[i == 0];
}
int main()
{
    fast_cin();
