...
ll findBest(v64& a, ll target){
    ll lo = 0, hi = sz(a), res = -1;
    while(lo <= hi){
        ll mid = lo + (hi - lo)/2;
        if(a[mid] >= target){
            hi = mid-1;
...
void solve(){
    ...
    sort(all(a), [&](v64& v1, v64& v2){
        return v1[1] < v2[1];
    });
    v64 vec; forn(i,n) vec.pb(a[i][1]);
    v64 dp(n+1);
    dp[0] = 0;
    for(ll i=1; i<=n; i++){
        ll op1 = dp[i-1]; // don't take
        ll op2 = a[i-1][2]; // take
        ll j = findBest(vec, a[i-1][0]);
        op2 += dp[j+1];
        dp[i] = max(op1, op2);
    }
    cout << dp[n] << ln;
