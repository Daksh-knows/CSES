...
void solve(){
    ...
    vector<Node> trie(1);
    function<void(ll)> add = [&](ll val){
        int idx = 0;
        for(ll i = 31; i>=0; i--){
            ll bit = (val>>i)&1;
            if(trie[idx].nxt[bit] == -1){
                trie[idx].nxt[bit] = sz(trie);
                trie.emplace_back();
            }
    ...
    add(0);
    function<ll(ll)> query = [&](ll val){
        ll idx = 0, tmp = 0;
        for(ll pw = 31; pw >= 0; pw--){
            ll bit = (val>>pw)&1;
            if(trie[idx].nxt[1 - bit] != -1) {
                tmp += (1LL << pw);
                idx = trie[idx].nxt[1 - bit];
            }else{
                idx = trie[idx].nxt[bit];
            }
        }
